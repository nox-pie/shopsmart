...
import { getExtraProducts } from '../data/extraProducts';
...
const mockProducts = [...mockCore, ...getExtraProducts()];
...
function notifyCartChanged() {
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new Event('shopsmart-cart-updated'));
  }
}
...
export async function addToCart(productId, quantity = 1) {
  try {
    ...
    const delta = Number.isFinite(qty) ? Math.trunc(qty) : 1;
    if (prod) {
      ...
      if (existing) {
        ...
        localCart.items.push({ product: prod, quantity: delta });
      }
